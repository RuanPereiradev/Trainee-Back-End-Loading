...
import { ProjectStatusType } from "@prisma/client";
  ...
  export class Project extends AuditableEntity{
    ...
    get status():ProjectStatusType {return this._status;}
    get goals(): string{return this._goals}
    

    public static create(name: string, sector: Sectors,status: ProjectStatusType,goals: string, description: string, id?: string): Result<Project> {
      if (!name.trim()) {
    ...
    changeStatus(newStatus: ProjectStatusType): Result<void>{
      if(!newStatus){
